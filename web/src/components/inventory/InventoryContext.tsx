...
const InventoryContext: React.FC = () => {
      ...
      case 'drop':
        if (isSlotWithItem(item)) {
          const count = Math.min(
            itemAmount > 0 ? itemAmount : item.count,
            item.count
          );
          dispatch(
            validateMove({
  ...
  return (
    <>
      <Menu>
        <MenuItem onClick={() => handleClick({ action: 'use' })} label={Locale.ui_use || 'Use'} data-action="use" />
        <MenuItem onClick={() => handleClick({ action: 'give' })} label={Locale.ui_give || 'Give'} data-action="give" />
